...
import pytz, cv2, pyttsx3, time, shutil, base64, json, threading, mtcnn, pickle, decimal
...
def mark_attendace(id1 ,user_image,status,img_time):
    attendace = False
    unknown_attendace = False
    if registration.objects.filter(id=id1).exists():
        reg_obj = registration.objects.get(id=id1)
        init_attendace = attendance.objects.filter(emp_id=reg_obj).order_by('-date_time').first()
        if init_attendace == None:
            retv, buffer1 = cv2.imencode('.jpg', user_image)
            ...
            prev_status=init_attendace.status
            prev_date= init_attendace.date_time
    ...
    if attendace == True:
        attendace_text = f'attendace marked for employee {id1}'
        return attendace_text
    if unknown_attendace == True:
        unknown_text = 'unknow user captured'
        return unknown_text
    if attendace == False:
        attendace_text = f"attendace already marked for the employee {id1}"
        return attendace_text
    

        # frame=frame[500:1500, 1000:2600]
        # img1 = img.copy()
        # cv2.rectangle(frame, (1000, 500), (2600, 1500), (0, 255, 0), 2)  #high resolution IN
...
def cam_in():
    ...
    l2_normalizer = Normalizer('l2')
    required_shape = (160, 160)
    face_encoder = InceptionResNetV2()  # Update with your model
    face_encoder.load_weights("facenet_keras_weights.h5")  # Update with your weights
    face_detector = cv2.FaceDetectorYN.create("yunet_22mar.onnx", "", (160, 160))  # Update with your detector
    line_thickness = 1
    edge_thickness = 3
    ...
    if not os.path.exists(csv_filename):
        # If the file doesn't exist, open it in write mode ('w') to create it and write the header row
        with open(csv_filename, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['id', 'IN_status', 'IN_time', 'Out_status', 'Out_time'])
    cap = cv2.VideoCapture(0)
    ...
    while cap.isOpened():
        ...
        if not ret:
            ...
            break
        dat=timezone.now()
        now_time = dat.strftime("%Y-%m-%d %H:%M:%S:%f")
        ...
        if faces is not None:
            for face in faces:
                ...
                cv2.putText(frame, id1 , (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
                    ...
                    try:
                        ...
                        if not existing_data:
                            with open(csv_filename, mode='a', newline='') as file:
                                writer = csv.writer(file)
                                writer.writerow([id1, status, now_time, None, None])
                            print("INITIAL Data Appended new row for ID:", id1)
                            info = mark_attendace(id1, frame, status, now_time)
                            print(info)
                        else:
                            if id1 not in existing_data:
                                print(f"No Data Available for the user {id1}")
                                with open(csv_filename, mode='a', newline='') as file:
                                    writer = csv.writer(file)
                                    writer.writerow([id1, status, now_time, None, None])
                                print("Appended new row for ID:", id1)
                                info = mark_attendace(id1, frame, status, now_time)
                                print(info)
                            else:
                                existing_data1 = {}
                                with open(csv_filename, mode='r') as file:
                                    reader = csv.DictReader(file)
                                    for row in reader:
                                        existing_data1[row['id']] = {
                                            'IN_status': row['IN_status'],
                                            'IN_time': row['IN_time'],
                                            'Out_status': row['Out_status'],
                                            'Out_time': row['Out_time']
                                        }
                                if existing_data1[id1]['IN_status'] is not None and existing_data1[id1]['Out_status'] == "":  # Check if Out_status is empty
                                    pass
                                else:
                                    # Append new row only if Out_status is not empty
                                    with open(csv_filename, mode='a', newline='') as file:
                                        writer = csv.writer(file)
                                        writer.writerow([id1, status, now_time, None, None])
                                    print("Appended new row for ID:", id1)
                                    # Assuming mark_attendance() is defined somewhere
                                    info = mark_attendace(id1, frame, status, now_time)
                                    print(info)
            ...
            pass
        retd3, frame3 = cv2.imencode('.jpg', frame)
        yield (b'--frame3\r\n'
                b'Content-Type: image/jpg\r\n\r\n' + frame3.tobytes() + b'\r\n')
        del frame, img_RGB, faces
        ...
        if faces is not None:
            for face in faces:
                ...
                try:
                    ...
                    # Find the row with the specific ID and empty Out_status and Out_time
                    mask = (df['id'] == id1) & (df['Out_status'].isna()) 
                    print(mask)
                    
                    if mask.any():
                        print(">>>>>>>>>>>>>>>>>>")
                        # Update the row with the new Out_status and Out_time
                        df.loc[mask, 'Out_status'] = status
                        df.loc[mask, 'Out_time'] = now_time
...
#                             pass
#         frame_count += 1
#         retd3, frame3 = cv2.imencode('.jpg', img)
#         yield (b'--frame3\r\n'
#                 b'Content-Type: image/jpg\r\n\r\n' + frame3.tobytes() + b'\r\n')
...
def calender_event_data(request):
    if request.method == "POST":
        ...
        start_date = datetime.strptime(startdate_str, '%Y-%m-%dT%H:%M:%S.%fZ').date()
        exect_date = start_date + timedelta(days=1)
        ...
        try:
            ...
            if event_data and not send_department and not source and "Present" in title:
                employee_ids = list(attendance.objects.filter(status='IN', date_time__date=exect_date).values_list('emp_id', flat=True).distinct())
...
def daily_attendance_page(request):
        ...
        for i in employee_present_list:
            ...
            if last_out_attendance.exists():
                ...
                emp_data.append({
                    ...
                    "date": exect_date,
                    "first_in": first_in_time_ist.strftime('%Y-%m-%d %I:%M:%S %p'),
                    "last_out": last_out_time_ist.strftime('%Y-%m-%d %I:%M:%S %p'),
                    "total": rounded_time,
                    "status": statuss
                })
            else:
    ...
    return render(request, "home/daily_attendance_report.html", {"present": present_count,
                                                                "absent": absent_count, "data": emp_data})
...
# def user_register(request):
#     ignored_id = 1
#     employee = registration.objects.all().order_by('-id').exclude(id=ignored_id)
#     context = {'data': []}
...
#         status = "IN"
#         dat = timezone.now()
#         dt_string1 = dat.strftime("%Y-%m-%d %H:%M:%S:%f")
#         recognition_t = 0.3
#         l2_normalizer = Normalizer('l2')
#         required_shape = (160, 160)
...
#                             emp_img_string1 = buffer2.tobytes()
#                             unknown.objects.create(status=status,image=emp_img_string1,date_time=dt_string1)
#                             print("first Unknown Person deteced with low confidence rate at the entrance camera")
#                         else:
#                             prev_status=u.status
#                             prev_time=u.date_time
#                             now_time=datetime.now().astimezone()
...
#                                 emp_img_string1 = buffer1.tobytes()
#                                 unknown.objects.create(status=status,image=emp_img_string1,date_time=dt_string1)
#                                 print("Todays first Unknown Person deteced with low confidence rate at the entrance camera")
#                     else:
#                         pass
 
#     queue = Queue()
#     recognition_process = Process(target=face_recognition_process, args=(queue,))
#     recognition_process.start()
...
#                 retd2, frame2 = cv2.imencode('.jpg', frame)
#                 yield (b'--frame1\r\n'
#                         b'Content-Type: image/jpg\r\n\r\n' + frame2.tobytes() + b'\r\n')
...
# def cam_out():
#     face_detector_out = cv2.FaceDetectorYN.create("yunet_22mar.onnx", "", (160, 160))
#     line_thickness_out = 1
#     edge_thickness_out = 1
   

#     def recognition_out(queue_out):
#         encodings_path_out = 'encodings/encodings.pkl'
#         encoding_dict_out = load_pickle(encodings_path_out)
#         status_out = "OUT"
#         dat_out = timezone.now()
#         dt_string2_out = dat_out.strftime("%Y-%m-%d %H:%M:%S:%f")
#         recognition_t_out = 0.3
#         l2_normalizer_out = Normalizer('l2')
#         required_shape_out = (160, 160)    
#         face_encoder_out = InceptionResNetV2()
#         face_encoder_out.load_weights("facenet_keras_weights.h5")
...
#                 face_img_out = arr_out[0]
#                 face_normalized_out = normalize(face_img_out)
#                 face_resized_out = cv2.resize(face_normalized_out, required_shape_out)
#                 face_d_out = np.expand_dims(face_resized_out, axis=0)
#                 encode_out = face_encoder_out.predict(face_d_out, verbose=False)
#                 encode_out = np.sum(encode_out, axis=0)
#                 encode_out = np.expand_dims(encode_out, axis=0)
#                 encode_out = l2_normalizer_out.transform(encode_out.reshape(1, -1))[0]
#                 min_distance_out = float('inf')
#                 id2 = '1'

#                 for db_name_out, db_encode_out in encoding_dict_out.items():
#                     dist_out = cosine(db_encode_out, encode_out)
#                     if dist_out < recognition_t_out and dist_out < min_distance_out:
#                         id2 = db_name_out
#                         min_distance_out = dist_out
...
#     queue_out = Queue() 
#     recog_out = Process(target=recognition_out, args=(queue_out,))
#     recog_out.start()
...
#         img_RGB_out = cv2.cvtColor(frame_out, cv2.COLOR_BGR2RGB)
#         height_out, width_out, _ = img_RGB_out.shape
#         face_detector_out.setInputSize((width_out, height_out))
#         _, faces_out = face_detector_out.detect(img_RGB_out)
#         if faces_out is not None:
#             for face_out in faces_out:
#                 face_confidence_out = float(face_out[-1])
#                 confidence_out = decimal.Decimal(face_confidence_out).quantize(decimal.Decimal('0.0001'), rounding=decimal.ROUND_DOWN)
...
#                 cv2.rectangle(frame_out, (x1_out, y1_out), (x2_out, y2_out), (0, 255, 0), line_thickness_out)
#                 # cv2.putText(frame, id2 , (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
#                 cv2.line(frame_out, (x1_out, y1_out), (x1_out + int(w_out/4), y1_out), (0, 255, 255), edge_thickness_out)
#                 cv2.line(frame_out, (x1_out, y1_out), (x1_out, y1_out + int(h_out/4)), (0, 255, 0), edge_thickness_out)
#                 cv2.line(frame_out, (x1_out + int(3*w_out/4), y1_out), (x2_out, y1_out), (0, 255, 0), edge_thickness_out)
#                 cv2.line(frame_out, (x2_out, y1_out + int(h_out/4)), (x2_out, y1_out), (0, 255, 255), edge_thickness_out)
#                 cv2.line(frame_out, (x1_out, y1_out + int(3*h_out/4)), (x1_out, y2_out), (0, 255, 255), edge_thickness_out)
#                 cv2.line(frame_out, (x1_out + int(w_out/4), y2_out), (x1_out, y2_out), (0, 255, 0), edge_thickness_out)
#                 cv2.line(frame_out, (x1_out + int(3*w_out/4), y2_out), (x2_out, y2_out), (0, 255, 255), edge_thickness_out)
#                 cv2.line(frame_out, (x2_out, y2_out - int(h_out/4)), (x2_out, y2_out), (0, 255, 0), edge_thickness_out)

#                 retd_out2, frame2_out = cv2.imencode('.jpg', frame_out)
#                 yield (b'--frame1\r\n'
#                         b'Content-Type: image/jpg\r\n\r\n' + frame2_out.tobytes() + b'\r\n')


#         retd_out3, frame3_out = cv2.imencode('.jpg', frame_out)
#         yield (b'--frame1\r\n'
#                 b'Content-Type: image/jpg\r\n\r\n' + frame3_out.tobytes() + b'\r\n')
...
#         ignored_id = 1
#         registrations = registration.objects.filter(attendance__date_time__startswith=today).exclude(id=ignored_id)
#         data = registrations.annotate(
#             first_in=Coalesce(Min('attendance__date_time', filter=models.Q(attendance__status='IN')), models.Value(None)),
#             last_out=Coalesce(Max('attendance__date_time', filter=models.Q(attendance__status='OUT')), models.Value(None)),
#             in_count=Count('attendance', filter=models.Q(attendance__status='IN')),
#             out_count=Count('attendance', filter=models.Q(attendance__status='OUT')),
#         ).values('id', 'name', 'department', 'first_in', 'last_out', 'in_count', 'out_count', )
#         context = {'data': data}
#         return render(request, 'home/reporting.html', context)
...
#             if last_out_attendance.exists():
#                 last_out_time = last_out_attendance.latest('date_time').date_time
#                 time_gap = (last_out_time - first_in_time).total_seconds() / 3600.0
#                 rounded_time = round(time_gap, 2)
#                 emp_data.append({
#                 "id":id,
#                 "name":name,
...
#     return render(request,"home/daily_attendance_report.html", {"present":present_count ,"absent":absent_count , "data":emp_data } )
